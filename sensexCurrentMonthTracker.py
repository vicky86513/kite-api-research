...
Automatically resolves the near-month SENSEX futures contract on BFO
(BSE F&O) — no need to hardcode the expiry each month. Writes to
sensex_price_book.xlsx (+ matching .json/.html), one row per unique strike
price, accumulating bid/ask quantity and refreshing the timestamp.

All log lines from this script go into trading_logs.txt (shared with the
MCX/Nifty trackers and auth.py) via logger.py, tagged with [SENSEX].

Run:
    python sensex_current_month_tracker.py

Requires auth.py, price_book.py, instruments.py, and logger.py in the same folder.
"""
...
EXCEL_FILE = "sensex_price_book.xlsx"
...
def run_tracker():
    ...
    writer = PriceBookWriter(EXCEL_FILE, title="SENSEX Current-Month Futures")
