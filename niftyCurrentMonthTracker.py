...
Automatically resolves the near-month NIFTY futures contract on NFO (no need
to hardcode the expiry each month). Writes to nifty_price_book.xlsx
...
MCX/Sensex trackers and auth.py) via logger.py, tagged with [NIFTY].

Run:
    python nifty_current_month_tracker.py
...
EXCEL_FILE = "nifty_price_book.xlsx"
...
def run_tracker():
    ...
    writer = PriceBookWriter(EXCEL_FILE, title="NIFTY Current-Month Futures")
